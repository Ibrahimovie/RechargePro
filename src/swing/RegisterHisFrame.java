 ...
 * @author kingfans
 */
public class RegisterHisFrame extends JFrame {
    private RechargeFrame rechargeFrame;
    private int count;
    private List<Map<String, Object>> his;
    private User user;

    public RegisterHisFrame(User user, int count, List<Map<String, Object>> his, RechargeFrame rechargeFrame) {
        try {
    ...
    private void initComponents(int n, List<Map<String, Object>> history) {
        ...
        String[] columnNames = {"卡号", "手机号", "姓名", "注册时间", "操作工号"};
            ...
            for (int j = 0; j < 5; j++) {
                switch (j) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        String regTime = String.valueOf(history.get(i).get("reg_time"));
                        obj[i][j] = regTime;
                        break;
                    case 4:
        ...
        scrollPane1.setBounds(10, 40, 835, 500);
...
//                    String registerJson = HttpUtils.toServlet(map, "card", "getRegisterHisWithoutPhone");
//                    JSONObject jsonObject = JSONObject.parseObject(registerJson);
//                    try {
...
//                    String registerJson = HttpUtils.toServlet(map, "card", "getRegisterHisWithPhone");
//                    JSONObject jsonObject = JSONObject.parseObject(registerJson);
//                    try {
//                        String hist = URLDecoder.decode(jsonObject.getString("register_his"), "utf-8");
//                        JSONObject object = JSONObject.parseObject(hist);
...
//                    }
                ...
                new RegisterHisFrame(user, count, his, rechargeFrame);
                this.dispose();
            } else {
        ...
        exportButton.setBounds(750, 8, 60, exportButton.getPreferredSize().height);
        ...
        exportButton.addActionListener(e -> {
            ...
            String[] title = new String[]{"卡号", "手机号", "姓名", "注册时间", "操作工号"};
            String fileName = "注册记录表" + sdf2.format(new Date()) + ".xls";
            String sheetName = "注册记录表";
            String[][] objects = new String[n][5];
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < 5; j++) {
                    switch (j) {
                        case 0:
